...
- **⚡ 工作流引擎** — 基于状态机的步骤化执行，动态权限控制，代码修改限制
...
| 命令 | 说明 |
|------|------|
| `/council start <topic>` | 启动议会辩论模式 |
| `/council stop` | 停止议会模式，返回自由模式 |
| `/council last` | 查看上次辩论结果 |
| `/council stats` | 查看模型能力统计 |

### 🔀 工作模式切换
| 命令 | 说明 |
|------|------|
| `/free` | 切换到自由模式（无约束，所有工具可用） |
| `/spec on` | 激活 Spec 模式（从文件加载规范） |
| `/spec on <content>` | 激活 Spec 模式（inline 内容） |
| `/spec off` | 停用 Spec 模式，返回自由模式 |
| `/spec edit` | 进入规范编辑模式 |
| `/spec show` | 显示当前规范内容 |
| `/spec clear` | 清除规范内容和文件 |
...
Ox 支持三种工作模式，通过工作流引擎统一管理，每种模式有不同的约束和权限。

### 三种工作模式

#### 1. **Free Mode（自由模式）** - 默认模式
- **工作流**: `free_workflow`（单步）
- **特点**: 无约束，开放式对话和编码辅助
- **权限**: 允许所有工具调用，包括代码修改
- **适用场景**: 日常对话、快速问答、自由探索
...
/free                    # 从其他模式切换
/spec off               # 停用 Spec 模式
/council stop           # 停用 Council 模式
```

#### 2. **Spec Mode（规范模式）** - 任务驱动开发
- **工作流**: `spec_workflow`（6步）
- **特点**: 基于规范的计划→审查→执行流程
- **权限**: 前期禁止代码修改，后期允许
- **适用场景**: 复杂功能开发、重构任务、需要严格规划的场景

**工作流程**:
```
Step 1: Topic Definition    → 定义任务主题
Step 2: Context Analysis    → 分析项目上下文
Step 3: Generate spec.md    → 生成规范文档（只读）
Step 4: Generate task.md    → 生成任务计划（只读）
Step 5: Review & Confirm    → 用户审查确认
Step 6: Execute             → 执行代码修改（允许写代码）
```

**激活方式**:
```bash
/spec on                     # 从 .ox/spec.md 加载
/spec on "实现用户认证"       # inline 内容
/spec edit                   # 交互式编辑规范
```

#### 3. **Council Mode（议会模式）** - 多智能体辩论
- **工作流**: `council_workflow`（6步）
- **特点**: 多模型辩论，提案→评审→反驳→仲裁
- **权限**: 禁止所有代码修改，仅生成讨论和建议
- **适用场景**: 架构决策、技术方案评估、复杂问题探讨

**工作流程**:
```
Step 1: Topic Definition    → 定义辩论主题
Step 2: Proposal Phase      → 各模型独立提案
Step 3: Review Phase        → 交叉评审打分
Step 4: Rebuttal Phase      → 反驳和修正
Step 5: Arbitration         → 仲裁得出最终结论
Step 6: Conclusion          → 总结和建议
```

**激活方式**:
```bash
/council start "递归 vs 迭代"
/council "如何设计缓存策略？"
```

### 模式对比表

| 特性 | Free Mode | Spec Mode | Council Mode |
|------|-----------|-----------|--------------|
| **工作流步骤** | 1步 | 6步 | 6步 |
| **代码修改** | ✅ 允许 | ⚠️ 仅Step 6 | ❌ 禁止 |
| **工具使用** | ✅ 全部 | ✅ 全部 | ✅ 全部 |
| **规划要求** | ❌ 无 | ✅ 强制 | ✅ 强制 |
| **适用场景** | 日常对话 | 任务开发 | 架构决策 |
| **UI 显示** | Step 1/1 | Step X/6 | Step X/6 |
...
if !step.allow_tool_execution {
    return Err("Tool execution not allowed in current step");
}

// 检查是否允许代码修改
if !step.allow_code_modification && is_code_tool {
...
#### 2. **源代码文件检测**
自动识别源代码文件（基于扩展名）：
- **代码文件**: `.rs`, `.py`, `.js`, `.ts`, `.java`, `.cpp`, `.go`, `.rb`, `.php`, `.swift`, `.kt` 等
- **文档文件**: `.md`, `.txt`, `.json`, `.toml`, `.yaml` 等（始终允许）

#### 3. **状态机管理**
```rust
pub enum WorkflowState {
    Free,                                    // 自由模式
    Spec { step: SpecWorkflowStep },        // Spec 模式（带步骤）
    Council {                               // Council 模式（带步骤和主题）
...
每帧 Tick 事件更新工作流状态显示：
```
[Spec Workflow] Step 3/6: Generate spec.md [████░░░░░░] 50%
💡 Current constraint: Code modification NOT allowed
```
...
```bash
# 初始状态：Free Mode
User: /spec on "实现用户登录功能"
→ 激活 spec_workflow
→ Step 1/6: Topic Definition
→ LLM 开始分析任务...

# 经过几个步骤后
[Spec Workflow] Step 6/6: Execute [██████████] 100%
💡 Code modification now ALLOWED
→ LLM 可以写入代码文件
```
...
User: /free
→ 立即切换到 free_workflow
→ Step 1/1: Free Interaction
→ 所有约束解除，回到自由模式
```

#### 场景 3: 议会辩论
```bash
User: /council start "Rust web framework selection"
→ 激活 council_workflow
→ Step 1/6: Topic Definition
→ 多个 AI 模型开始辩论...
→ 全程禁止代码修改，仅生成建议
...
1. **异步安全锁**: 全部使用 `try_lock()` 避免阻塞 tokio 运行时
2. **缓存优化**: UI 显示数据缓存，避免渲染时加锁
3. **优雅降级**: 获取锁失败时记录警告而非 panic
4. **向后兼容**: 保留 deprecated 字段确保平滑迁移

### 架构优势

- ✅ **统一管理**: 三种模式都通过 workflow 引擎管理，架构一致
- ✅ **灵活扩展**: 易于添加新的工作流模式
- ✅ **权限细粒度**: 基于步骤的动态权限控制
- ✅ **用户体验**: 实时显示当前状态和约束
- ✅ **安全可靠**: 防止意外的代码修改
...
| **状态机** | WorkflowEngine + SessionState |
...
- **[Workflow Engine 完整调用流程](docs/Workflow_Engine_完整调用流程.md)** - 详细的工作流引擎架构和调用链路文档
- **设计文档** - 查看 `docs/` 目录下的其他技术设计文档
