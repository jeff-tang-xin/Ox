        ...
        match event_type {
            "message_start" => {
                if let Some(usage) = json.get("message").and_then(|m| m.get("usage")) {
                    self.prompt_tokens = usage.get("input_tokens").and_then(|v| v.as_u64()).unwrap_or(0) as u32;
                }
            }
            "content_block_start" => {
                ...
                if let Some(content_block) = json.get("content_block") {
                    ...
                    if block_type == "tool_use" {
                        ...
                        if !name.is_empty() {
                            ...
                            events.push(LlmStreamEvent::ToolCallStart { id, name: name.to_string() });
                        }
                    }
                }
            }
            ...
            "error" => {
                ...
                events.push(LlmStreamEvent::Error(format!("[{}] {}", error_type, error_msg)));
            }
            _ => {}
        }
...
mod tests {
    ...
    fn test_adapter_factory() {
        ...
        assert!(events.iter().any(|e| matches!(e, LlmStreamEvent::TextDelta(s) if s == "test")));
