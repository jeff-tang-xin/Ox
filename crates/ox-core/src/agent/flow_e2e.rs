...
use super::findings::{Finding, FindingsStore, FindingStatus, Severity};
...
use super::workflow::{create_default_workflow, DEFAULT_WORKFLOW_ID};
...
fn active_engine() -> WorkflowEngine {
    ...
    engine.set_variable("_current_user_request", "审查 MaintainDeliveryStrategy".into());
    engine
}
...
fn review_round_starts_in_review_phase() {
    ...
    assert!(matches!(ws.required_action, RequiredAction::EmitFindingsAndDone));
}
...
fn review_done_without_receipt_stays_await_user() {
    ...
    phase::on_round_started(&engine, TaskIntent::Review);
    seed_findings(&engine);
    phase::transition(&engine, PhaseEvent::DoneGatePassed {
...
fn broad_explore_blocked_in_implement() {
    ...
    assert!(engine.validate_tool_call("code_search", &args).is_err());
    assert!(engine.validate_tool_call("find_symbol", &args).is_ok());
    assert!(engine
        .validate_tool_call("recall", &serde_json::json!({"node_id": "step-1"}))
        .is_ok());
    assert!(engine
        .validate_tool_call(
            "memory_search",
            &serde_json::json!({"query": "error handling conventions", "max_results": 5})
        )
        .is_ok());
    assert!(engine
        .validate_tool_call(
            "memory_search",
            &serde_json::json!({"query": "read Foo.java line 10"})
        )
        .is_err());
}
...
fn scope_confirm_clears_turn_state_on_enter_implement() {
    ...
    phase::on_scope_selected(&engine);
    assert_eq!(phase::get(&engine), SingleFlowPhase::Implement);
    assert!(engine.get_variable("_turn_memory").unwrap_or_default().is_empty());
    assert_eq!(
