...
impl WorkflowEngine {
    ...
    pub fn request_confirmation(&self, message: &str) -> Option<InterventionRequest> {
        if let Some(step) = self.current_step() {
            if let Ok(session) = self.session_state.try_lock() {
                let request = InterventionRequest::confirmation(
                    message,
                    &step.name,
        ...
        if let Ok(mut session) = self.session_state.try_lock() {
            if session.advance_step(workflow.total_steps()) {
                tracing::info!(
                    ...
                    workflow.get_step(session.current_step_index)
                        .map(|s| s.name.as_str())
                        .unwrap_or("Unknown")
                );
                Ok(true)
    ...
    pub fn validate_tool_call(&self, tool_name: &str, args: &serde_json::Value) -> Result<(), String> {
        let step = self.current_step()
            .ok_or("No active workflow step")?;
        
        // Check if tools are allowed at all
        if !step.allow_tool_execution {
            return Err(format!("Tool execution not allowed in current step: {}", step.name));
        }
