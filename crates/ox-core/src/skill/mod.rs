...
pub mod policy; // Loading tiers + mandatory project skills // Duplicate prevention + merge
...
impl SkillLoader {
    ...
    pub fn load_enabled_skills(&self) -> anyhow::Result<Vec<Skill>> {
        ...
        if self.project_skills_dir.exists() {
            user_skills
                .extend(self.load_skills_from_dir(&self.project_skills_dir, SkillScope::Project)?);
        }

        if self.global_skills_dir.exists() {
            user_skills
                .extend(self.load_skills_from_dir(&self.global_skills_dir, SkillScope::Global)?);
        }
...
pub fn parse_phases_from_metadata(
    metadata: &std::collections::HashMap<String, String>,
) -> Vec<String> {
    metadata
        .get("phases")
...
pub fn skill_applies_to_phase(skill: &Skill, phase: &str) -> bool {
    ...
    skill
        .phases
        .iter()
        .any(|sp| sp == &p || (p == "receive" && sp == "review"))
}
