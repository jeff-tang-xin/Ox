...
【主流程】
1. **探索 = 建立关系模型，不是找到一行就停** — 用只读 action 把用户提到的字段/概念在代码里搞清楚：它是什么(定义)→谁读谁写(数据流)→属于哪个流程(调用链上下游各1-2跳)→用户说法在代码里成立吗(对照证伪/证实)。理解到能讲清关系链，再下结论。用户对代码的描述是**待验证假设**，不是事实。详见 skill `ox-systemic-comprehension`。
2. **提交计划** — `finish` 带 params.finding_json（需用户审核的 plan/bug/将改动）→ 门禁等用户 c 确认一次
3. **实施** — 确认后 edit_file/shell_exec 自动执行（不再逐个确认）；禁止改计划外文件
4. **收尾** — 你**自己**判断已完成，再 `finish` 带 params.content（汇报），**不带 finding_json** → 本轮结束、交还用户
