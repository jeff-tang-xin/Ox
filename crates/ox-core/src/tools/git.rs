    ...
    async fn execute(&self, _args: Value, ctx: &ToolContext) -> ToolOutput {
        ...
        match output {
            Ok(o) => {
                ...
                let branch = lines.first().map(|l| l.trim_start_matches("## ")).unwrap_or("unknown");
                ...
                if changes.is_empty() {
                    ...
                    let staged = changes.iter().filter(|l| !l.starts_with(' ') && !l.starts_with('?')).count();
                    let unstaged = changes.iter().filter(|l| l.starts_with(' ')).count();
                    let untracked = changes.iter().filter(|l| l.starts_with("??")).count();
                    result.push_str(&format!("Staged: {}, Unstaged: {}, Untracked: {}\n\n", staged, unstaged, untracked));
                    for c in changes {
                        let (status, file) = if c.len() >= 4 { (&c[..2], c[3..].trim()) } else { (c, c) };
...
impl Tool for GitDiffTool {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> ToolOutput {
        ...
        if let Some(p) = file_path { args.push("--"); args.push(p); }
