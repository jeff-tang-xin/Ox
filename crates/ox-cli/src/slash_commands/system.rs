...
pub const PLAN_COMMAND: CommandMeta = CommandMeta {
    name: "plan",
    aliases: &[],
    description: "Show task plan status",
    handler: handle_plan,
};
        ...
        match runtime::change_directory(rt_env, &path) {
            DirectoryChangeResult::Success {
                ...
                if project_changed {
                    ...
                    app.output
                        .push_system(&format!("Project boundary changed -- {}", name));
                    ...
                    if ox_core::agent::onboarding::needs_project_onboarding(&proj_root) {
                        ...
                        app.output
                            .push_system("   → project-conventions.md + project-business-guide.md");
                        ...
                        app.output
                            .push_system("📋 项目 Skill 已存在 — 已从缓存加载。");
                    }
                }
            }
...
pub fn handle_init(
    ...
    match OxConfig::init_default_config() {
        Ok(path) => app.output.push_system(&format!(
            "Config created at {}. Edit it to add API keys.",
            path.display()
        )),
...
pub fn handle_debug(
    ...
    app.output
        .push_line(OutputLine::System(format!("Model: {}", app.model_name)));
...
pub fn handle_reload(
    ...
    match Session::load(&session.dir().to_path_buf()) {
        ...
        Ok(None) => app
            .output
            .push_error("Failed to reload: session file is empty or corrupted"),
        Err(e) => app
            .output
            .push_error(&format!("Failed to reload session: {}", e)),
    }
    CommandResult::Success
}
